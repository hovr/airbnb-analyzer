...
const UPDATE_ICON_PATHS = {
  16: 'icon-update16.png',
  32: 'icon-update32.png',
  48: 'icon-update48.png',
  128: 'icon-update128.png'
};
...
async function setVersionUpdateBadge(hasUpdate) {
  versionUpdateAvailable = Boolean(hasUpdate);
...
async function restoreVersionUpdateIconState() {
  try {
    ...
    if (status?.currentVersion === currentVersion) {
      versionUpdateAvailable = Boolean(status.hasUpdate);
    }
  } catch (error) {
...
restoreVersionUpdateIconState().then(() => {
  if (versionUpdateAvailable) {
...
async function scrollAndLoadAllReviews(expectedTotal, propertyNumber = null, totalProperties = null) {
  ...
  for (let cycle = 1; cycle <= MAX_SCROLL_CYCLES; cycle += 1) {
    ...
    if (currentCount > lastCount) {
      ...
      if (stagnantMovementCycles >= 5) {
        console.warn('No scroll movement detected over multiple cycles, treating as complete');
        expected = currentCount || expected || null;
        break;
      }
      if (idleCycles > MAX_IDLE_CYCLES) {
